...
# Map them
ggplot(locations, aes(lon_deg, lat_deg, color = month.abb[lubridate::month(Date)])) +
  geom_point() +
  theme_minimal() +
  map_theme() + 
  labs(
...
location_bottemps %>% 
  ...
  labs(x = "Recorded Surface Temperature",
       y = "GLORYS Bottom Temperature")
...
nrow(locations) - nrow(location_bottemps)
```
...
location_bottemps %>% 
  st_drop_geometry() %>% 
  write_csv(here::here("local_data/baukus_trip_positions/Sensor Trip Position BT Data.csv"))
```
