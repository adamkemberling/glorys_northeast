...
# Seaweed Saturday Prep

Kelp/seaweed species play an important role in the region's ecology roles are sessile organisms that are unable to move


Chat gpt says a very similar 5-15C, we could use a 18C Threshold.

Depth is a critical thing for Kelp, they typically exist at depths less than 40m, and need rocky substrates.
...
# Show the temp  preference
plot(
  kelp_beta ~ temp_range, 
  type = "l", 
  main = "Hypothetical Kelp Temperature Preference Curve", 
  ylab = "Preference", 
  xlab = "Temperature Range")
...
# Apply the preferences
kelp_prefs <- map(temp_decades, function(decade_x){
  map(
    setNames(c("sst", "bt"), 
             c("sst", "bt")), 
    function(x){
...
bt_suit_1990s <- ggplot() +
  geom_spatraster(
    data = kelp_prefs$avg_1990$bt) +
  geom_spatraster_contour(
    data = kelp_prefs$avg_1990$bt, 
    breaks = seq(0,1,.25), color = "white", linewidth = 0.15) +
  geom_sf(data = new_england) +
...
bt_suit_2000s <- ggplot() +
  geom_spatraster(
    data = kelp_prefs$avg_2000$bt) +
  geom_spatraster_contour(
    data = kelp_prefs$avg_2000$bt, 
    breaks = seq(0,1,.25), color = "white", linewidth = 0.15) +
  geom_sf(data = new_england) +
  ...
  guides(fill = guide_colorbar(barwidth = unit(4, "cm"))) +
  labs(
    title = "2000's", fill = "Temperature Suitability")
...
bt_suit_2010s <- ggplot() +
  geom_spatraster(
    data = kelp_prefs$avg_2010$bt) +
  geom_spatraster_contour(
    data = kelp_prefs$avg_2010$bt, 
    breaks = seq(0,1,.25), color = "white", linewidth = 0.15) +
  geom_sf(data = new_england) +
...
```{r}
(bt_suit_1990s | bt_suit_2000s | bt_suit_2010s) + plot_layout(guides = "collect") + plot_annotation(title = "Kelp Bottom Temperature Suitability")
```
...
map_09 <- ggplot() +
  geom_spatraster(
    data = bot_suitability[["2009-02-28"]]) +
  geom_spatraster_contour(
    data = bot_suitability[["2009-02-28"]], 
    breaks = seq(0,1,.25), color = "white", linewidth = 0.15) +
  geom_sf(data = new_england) +
  ...
  guides(fill = guide_colorbar(barwidth = unit(4, "cm"))) +
  labs(
    title = "Winter Kelp Temperature Suitability\nFebruary 2009",
    subtitle = "Kelp Suitability Range of 10-20C",
    fill = "Temperature Suitability")

map_10 <- ggplot() +
  geom_spatraster(
    data = bot_suitability[["2010-05-31"]]) +
  geom_spatraster_contour(
    data = bot_suitability[["2010-05-31"]], 
    breaks = seq(0,1,.25), color = "white", linewidth = 0.15) +
  geom_sf(data = new_england) +
